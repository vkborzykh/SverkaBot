# SverkaBot — Database Draft v4.2

This document supersedes DB Draft v4.1 and defines the canonical PostgreSQL schema for SverkaBot.
...
**Changelog v4.1 → v4.2 (2026-07):**
- `canonical_transactions.category` added — populated by the WB deduction-column
  detector (`src/lib/jobs/handlers/parseWb.ts`, `DEDUCTION_COLUMNS`) for OUT-direction
  rows: `LOGISTICS`, `STORAGE`, `PENALTY`, `DEDUCTION`, `COMMISSION`, `MARKETING`, or
  `NULL` when uncategorized. Used to build the deduction breakdown shown in the
  underpayment message (`getWbDeductionsByCategory` in `reconcile.ts`) and, longer
  term, in reports.
- Documented that `reconciliation_candidates` (§14a) is now populated for every
  completed run, not only during manual QA — see §27a for the row-level shadow
  engine status.
- This revision does **not** yet reflect the actual current pricing/tariff model
  (START/PRO/BUSINESS + export add-on + referral), which still diverges from
  PRD §5. That sync is a separate, larger pass — tracked, not done in this revision.
...
`category` (added v4.2): nullable, free text (not an enum — detector categories may
evolve without a migration). Populated only for OUT-direction WB rows where the
report's column headers matched a known deduction pattern. `NULL` means either an
IN-direction row (sales/payouts) or an OUT-direction row whose deduction type
could not be confidently classified from the report's column headers.
...
## 27a. Reconciliation engine status (added v4.2)

Two engines coexist in the codebase; only one is user-facing:

- **Aggregate engine (production, primary)** — `src/lib/reconciliation/wbPayoutCore.ts`
  (`reconcileWbPayout`). Computes `Σ WB IN − Σ WB OUT` against matched bank credits.
  This is the only engine whose output is written to `reconciliation_runs`
  (`matched_count`, `loss_kopeks`, etc.) and shown to users.
- **Row-level engine (shadow only, not user-facing)** — `candidates.ts` → `scoring.ts`
  (`updateCandidateScores`) → `assignment.ts` (`globalMatch`) → `splitCombined.ts`.
  Runs automatically after every completed reconciliation (`runRowLevelShadow` in
  `reconcile.ts`), fire-and-forget, errors are caught and logged to
  `admin_notifications` — never surfaced to the user and never blocks the job.
  `generateCandidates()` **does** persist to `reconciliation_candidates` regardless
  of shadow/dry-run mode; `globalMatch(runId, { dryRun: true })` does not persist
  to `reconciliation_matches`/`reconciliation_match_items`.

**Practical consequence:** `reconciliation_candidates` rows exist for every
completed run and are safe to query directly (used by `claimBuilder.ts` to build
the row-level "claim" section of exports/reports — see below). `reconciliation_matches`
rows do **not** reliably exist for shadow-mode runs and should not be assumed present.

**Cutover criteria (not yet met as of this revision):** the row-level engine
becomes the primary, user-facing path only after its output has been compared
against the aggregate engine's output over a sustained period of real traffic
(via `admin_notifications` shadow-run logs) with acceptable agreement. Until then,
any feature that reads `reconciliation_matches`/`_match_items`/`_evidence` should
not assume they are populated.

## 27b. Row-level claim generation (added v4.2)

`src/lib/reconciliation/claimBuilder.ts` (`buildRowLevelClaim`) builds a per-row
breakdown of specific unpaid WB transactions for the HTML report, XLSX "Претензия"
sheet, and 1С registry — **without** depending on the row-level engine's
scoring/assignment/split-combined stages (those remain shadow-only, see §27a).

It works off `reconciliation_candidates` alone: a WB IN-direction transaction with
**zero** candidates (no bank transaction of the same amount within the date window)
is treated as a high-confidence "genuinely unpaid" line item. The resulting sum is
cross-checked against the trusted aggregate `loss_kopeks`; if they diverge beyond a
5% / 500₽ tolerance, the row-level breakdown is discarded and callers fall back to
the pre-v4.2 aggregate-only claim text. This makes the feature safe to ship ahead
of the full row-level engine cutover in §27a.

## 28. Migration notes from MVP
...
9. add `canonical_transactions.category` (v4.2);
10. reconcile documented pricing/tariff model with actual implementation (pending — see changelog).

End of DB Draft v4.2.
